...
//ModelsPath = GetCurrentDir()+"\\Data\\Models\\";
//TexturesPath = ModelsPath;
        // 3D-объект
Mesh = NULL;
SizeOfVertex = 0;
...
//---------------------------------------------------------------------------
bool Object3D::LoadFromFile(AnsiString File)
{
        // Загрузка объекта из файла
if(FileExists(BasePath+File)==false) File = AlternativeFile;
LPD3DXBUFFER MatBuffer;  // Временный буфер для сохранения материалов объекта
D3DXLoadMeshFromX((BasePath+File).c_str(),D3DXMESH_SYSTEMMEM,pD3DDevice,NULL,&MatBuffer,&MaterialsCount,&Mesh);
...
for(DWORD i=0; i<MaterialsCount; i++) {
        ...
        if(FAILED(D3DXCreateTextureFromFile(pD3DDevice,(ExtractFilePath(BasePath+File)+AnsiString(TmpMaterials[i].pTextureFilename)).c_str(),&Textures[i]))) Textures[i] = NULL;
        }
MatBuffer->Release();
return true;
}

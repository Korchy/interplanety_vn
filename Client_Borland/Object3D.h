...
protected:      // Доступно здесь и для потомков
        ...
        AnsiString AlternativeFile;     // Альтернативный файл, который загружается в LoadFromFile, если указанного файла не существует        

private:        // Доступно только здесь
        // Переменные
//        AnsiString ModelsPath;          // Путь к файлам моделей
//        AnsiString TexturesPath;        // Путь к файлам текстур к моделям


public:         // Доступно везде
        Object3D(void); // Констуктор класса без параметров
        ~Object3D();    // Деструктор класса

        // Переменные


        // Функции
        bool LoadFromFile(AnsiString File);     // Загрузка объекта из файла
